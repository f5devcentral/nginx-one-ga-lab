...
The **Lab Framework** from the UDF environment has **Docker** installed and is setup so it can run an NGINX Plus container image. The container image we will use here is **private-registry.nginx.com/nginx-plus/agent:r36-debian** which has NGINX Plus with the Agent. If you want to see a list of all NGINX Plus with Agent image tags, refer to the [documentation](https://docs.nginx.com/nginx/admin-guide/installing-nginx/installing-nginx-docker/#pulling-the-image).
...
command below, take the following actions:
    ...
    -d private-registry.nginx.com/nginx-plus/agent:r36-debian
    ```
