...
1. To start your NGINX instance, click the **Run** button in this lab guide to deploy and have it automatically connect to the NGINX One Console:
   ...
   <SetVariable name="nginx-plus-4" isComponent="true" />

   > Note: If you receive an error, it may be that you have not input and saved the **NGINX_AGENT_COMMAND_AUTH_TOKEN** variable above.
...
In this section, we will configure our **<GetVariable name="nginx-plus-4" />** NGINX instance to leverage an SSL/TLS certificate that already exists on the instance.

1. In the NGINX One console, select _Manage_ and then _Instances_ from the left menu.

1. You should see your **<GetVariable name="nginx-plus-4" />** instance in the list. Click it to view the instance details.
...
1. To check that the **<GetVariable name="nginx-plus-4" />** instance is now listening on port 443, click the _Check_ button below.<br />

    <APICheck
      componentName="nginx-plus-4"
...
### Test the Configuration
  ...
  To check that the **<GetVariable name="nginx-plus-4" />** instance is now listening on 8443, click the _Check_ button below.

   <APICheck
     url={`https://${process.env["PETNAME"]}-nginxplus4:8443`}
     targetStatusCode={200}
   />
...
Through the use of the Certificate Management features in NGINX One Console, you have now accomplished common tasks such as viewing the status of existing certificates, and uploading new or updated certificates. The capability to perform this function within Config Sync Groups is also available to save operational toil associated with updating certificates across a cluster of NGINX instances. You can also perform these functions via the [NGINX One Console API](https://docs.nginx.com/nginx-one/api/api-reference-guide/) if you prefer to handle these steps through a pipeline.
...
1. Wait until your **<GetVariable name="petname" />-nginxplus4** instance transitions into the **Unavailable** state before proceeding. You may need to click the _Refresh_ button at the top right of the instance list.

1. On the **<GetVariable name="petname" />-nginxplus4** line of the instances table, select the _Actions_ "..." context menu, and select
   _Delete_.
